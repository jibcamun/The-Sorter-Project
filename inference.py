...
def _clamp_final_score(value: float) -> float:
    return max(MIN_FINAL_SCORE, min(MAX_FINAL_SCORE, float(value)))


def _format_final_score(value: float, decimals: int = 2) -> str:
    return f"{_clamp_final_score(value):.{decimals}f}"
...
def build_system_prompt(task_name: str) -> str:
    return textwrap.dedent(
        ...
        c) adjust: return a single tuple of the form ["object_name", option_index]. The option_index selects one of the exposed legal target options for that object. Option indices are recomputed from the current state on every step, so do not count upward across steps. Pick the best current option based on the exposed target details, usually the option with the highest value_delta and often index 0.
...
def log_step(
    ...
    print(
        f"[STEP] step={step} action={action} reward={_format_final_score(reward)} done={done_val} error={error_val}",
        flush=True,
    )


def log_end(success: bool, steps: int, score: float, rewards: List[float]) -> None:
    final_score = _clamp_final_score(score)
    rewards_str = ",".join(_format_final_score(r) for r in rewards)
    print(
...
def build_user_prompt(
    ...
    if task_name == "adjust":
        adjust_guidance = (
            ...
            "Do not walk through 0, 1, 2, 3 across turns unless the current exposed value_delta data supports that choice. "
...
def run_task_phase(
    ...
    try:
        ...
        if final_grade is not None:
            score = _clamp_final_score(final_grade.normalized_score)
